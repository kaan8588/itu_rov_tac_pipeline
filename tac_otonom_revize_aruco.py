...
class Smoother:
    ...
    def reset(self):
        self.buf.clear()
class ArucoPassConfirmer:
    """
    Fiziksel geçiş sırasına göre ArUco marker onaylayıcı.

    ── Temel Matematik ──────────────────────────────────────────────────────
    OpenCV'de Y ekseni yukarıdan aşağıya artar (üst=0, alt=frame_h).
    Kamera hafif aşağı baktığı için:
        • Küçük Y  → marker uzakta  (ekranın üstü)  → henüz yaklaşılmadı
        • Büyük Y  → marker yakında (ekranın altı)   → araç geçmek üzere

    ── Üç Aşamalı Onay Süreci ──────────────────────────────────────────────
    Aşama 1 — Y Eşiği Filtresi:
        center_y < y_threshold  →  marker çok uzakta, tamamen yoksay.
        Bu filtre sayesinde boru kıvrımlarında ilerideki marker'lar
        erken aktife alınmaz.

    Aşama 2 — sight_confirm (Titreme Koruması):
        Y eşiğini geçen marker art arda `sight_confirm` frame görülmeden
        aktife alınmaz. Kamera titremesi veya köşe dönüşlerinde oluşan
        1-2 frame'lik yanlış pozitifler bu sayede elenir.

    Aşama 3 — pass_confirm (Geçiş Onayı):
        Aktif marker art arda `pass_confirm` frame boyunca Y eşiğinin
        altında görülmezse araç onu fiziksel olarak geride bırakmıştır
        → ordered listesine eklenir.

    ── Tie-breaker (Aynı Anda Geçiş) ──────────────────────────────────────
    Birden fazla marker aynı frame'de pass_confirm'e ulaşırsa:
        1. Öncelik: miss_count (büyük = daha önce kayboldu = daha önce geçildi)
        2. Eşitlik:  max_y     (büyük = araca daha çok yaklaşmıştı = önce geçildi)

    Parametreler:
        y_threshold   : bu Y değerinin altındaki marker'lar işleme alınır
                        (0.0–1.0 arası normalize, frame_h ile çarpılır)
                        Varsayılan: 0.5 → frame'in alt yarısı
        sight_confirm : aktife almak için gereken ardışık görülme sayısı
                        Varsayılan: 5
        pass_confirm  : geride bırakma için gereken ardışık kayıp frame
                        Varsayılan: 12
    """
    ...
    def __init__(self,
        ...
        self.y_threshold   = y_threshold    # normalize (0.0–1.0)
        self.sight_confirm = sight_confirm
        self.pass_confirm  = pass_confirm

        # ── İç durum sözlükleri ──────────────────────────────────────────
        # sight_counts[id] : Y eşiğini geçtikten sonra art arda kaç frame görüldü
        self.sight_counts: dict[int, int]   = {}
        # active_markers[id] = {'miss_count': int, 'max_y': float}
        #   miss_count : art arda kaç frame Y eşiğinin altında görülmedi
        #   max_y      : aktifken ulaşılan en yüksek Y değeri (tie-breaker)
        self.active_markers: dict[int, dict] = {}
        # seen : ordered'a eklenmiş ID'ler — tekrar eklemeyi engeller
        self.seen:           set[int]        = set()

        # ── Dışarıya açık sonuç alanları ────────────────────────────────
        self.ordered:  list[int] = []   # fiziksel geçiş sırasıyla onaylanmış ID'ler
        self.new_ids:  list[int] = []   # bu frame'de yeni onaylananlar (log için)

    # ── Ana güncelleme metodu ────────────────────────────────────────────
    def update(self,
               detections: dict[int, float],
               frame_h:    int) -> list[int]:
        """
        Her frame'de çağrılır.

        Parametreler:
            detections : {marker_id: center_y}  — bu frame'deki TÜM tespitler.
                         Y eşiği filtresi bu metod içinde uygulanır.
            frame_h    : frame yüksekliği (piksel) — eşik hesabı için gerekli.

        Döndürür:
            ordered    : güncel fiziksel sıra listesi.
        """
        self.new_ids = []
        y_thresh_px  = self.y_threshold * frame_h

        # Y eşiğini geçen (araca yeterince yakın) tespitler
        valid: dict[int, float] = {
            mid: cy
            for mid, cy in detections.items()
            if cy >= y_thresh_px and mid >= 0
        }

        # ── Aşama 1: sight_confirm — Y eşiğini geçenleri say ────────────
        for mid, cy in valid.items():
            if mid in self.seen:
                continue

            if mid in self.active_markers:
                # Zaten aktif: miss sayacını sıfırla, max_y güncelle
                self.active_markers[mid]['miss_count'] = 0
                self.active_markers[mid]['max_y'] = max(
                    self.active_markers[mid]['max_y'], cy)
            else:
                # Henüz aktif değil: sight sayacını artır
                self.sight_counts[mid] = self.sight_counts.get(mid, 0) + 1
                if self.sight_counts[mid] >= self.sight_confirm:
                    # sight_confirm doldu → aktife al
                    self.active_markers[mid] = {'miss_count': 0, 'max_y': cy}
                    self.sight_counts.pop(mid)

        # ── Aşama 2: pass_confirm — aktif ama görülmeyenleri say ─────────
        passed_this_frame: list[dict] = []

        for mid in list(self.active_markers):
            if mid in valid:
                continue  # bu frame'de görüldü, atla

            self.active_markers[mid]['miss_count'] += 1

            if self.active_markers[mid]['miss_count'] >= self.pass_confirm:
                # Geride bırakıldı
                passed_this_frame.append({
                    'id':        mid,
                    'miss_count': self.active_markers[mid]['miss_count'],
                    'max_y':     self.active_markers[mid]['max_y'],
                })
                del self.active_markers[mid]

        # ── Aşama 3: Tie-breaker ile sırala ve kaydet ────────────────────
        if passed_this_frame:
            # miss_count büyük → daha önce kayboldu → daha önce geçildi
            # max_y büyük     → araca daha çok yaklaşmıştı → daha önce geçildi
            passed_this_frame.sort(
                key=lambda x: (x['miss_count'], x['max_y']),
                reverse=True)

            for p in passed_this_frame:
                self._add_to_sequence(p['id'])

        return self.ordered

    # ── Yardımcı metotlar ────────────────────────────────────────────────
    def _add_to_sequence(self, mid: int):
        """Marker'ı ordered listesine ekle (tekrar ekleme korumalı)."""
        if mid not in self.seen:
            self.seen.add(mid)
            self.ordered.append(mid)
            self.new_ids.append(mid)

    def flush_active(self) -> list[int]:
        """
        Görev bitişinde pass_confirm dolmadan aktif kalan marker'ları
        zorla onayla. Boru sonu / ST_SURFACE geçişinde çağrılmalı.

        Sıralama kuralı mevcut pass_confirm ile aynı:
            1. miss_count büyük → önce
            2. Eşitlikte max_y büyük → önce
        """
        ...
        remaining = [
            {'id': mid,
             ...
             'max_y':      v['max_y']}
            for mid, v in self.active_markers.items()
            if mid not in self.seen
        ]
        remaining.sort(
            key=lambda x: (x['miss_count'], x['max_y']),
...
class TacAutonomousNode(Node):
    ...
    def __init__(self):
        ...
        self.confirmer       = ArucoPassConfirmer(
                                    y_threshold=self.aruco_y_threshold,
                                    sight_confirm=self.aruco_sight_confirm,
                                    pass_confirm=self.aruco_pass_confirm)
    ...
    def _declare_params(self):
        ...
        self.declare_parameter('aruco_y_threshold',   0.5)   # frame alt yarısı (0.0-1.0)
        self.declare_parameter('aruco_sight_confirm', 5)     # aktife almak için min görülme
        self.declare_parameter('aruco_pass_confirm',  12)    # geride bırakma için min kayıp
    ...
    def _read_params(self):
        ...
        self.aruco_y_threshold   = float(g('aruco_y_threshold').value)
        self.aruco_sight_confirm = g('aruco_sight_confirm').value
        self.aruco_pass_confirm  = g('aruco_pass_confirm').value
    ...
    def run(self):
        ...
        try:
            while self._running and rclpy.ok():
                ...
                # Tüm tespitleri {id: center_y} formatında topla.
                # Y eşiği filtresi ArucoPassConfirmer içinde uygulanır.
                detections: dict[int, float] = {}
                ...
                # ids_flat: state machine için (mevcut arayüz korunuyor)
                ids_flat = list(detections.keys())

                # Boru sonu yaklaşıyorsa aktif marker'ları zorla onayla
                if self.pipe_end_cnt >= max(1, self.pipe_end_limit // 2):
                    flushed = self.confirmer.flush_active()
                    for mid in flushed:
                        self.get_logger().info(
                            f"  MARKER ID {mid:3d} flush ile onaylandı (boru sonu yakın)")

                ordered = self.confirmer.update(detections, self.frame_h)

                for mid in self.confirmer.new_ids:
                    self.get_logger().info(
                        f"  MARKER ID {mid:3d} geride bırakıldı ✓ | "
                        f"Sıra: {','.join(str(x) for x in ordered)}")
                ...
                if prev_state != ST_SURFACE and self.state == ST_SURFACE:
                    # Geride kalmış ama henüz pass_confirm dolmamış marker'ları onayla
                    flushed = self.confirmer.flush_active()
                    for mid in flushed:
                        self.get_logger().info(
                            f"  MARKER ID {mid:3d} SURFACE flush ile onaylandı")
